...
class Perdestrian extends Scene{
	...
	if($h_res == FALSE){
        ...
        $error = array('status' => "Failed", "msg" => "Request to create a scene was successfull.");
        $this->api->response($this->api->json($error), 400);
    ...
    public function getPedestrian($perdestrianID) {
        try{
            ...
            while(($array = mysql_fetch_array($h_res)))
            {
                ...
                $h_array['caseData'] = $this->case->getCaseByScene($h_array['sceneID']);
                $h_array['sceneData'] = $this->getSceneByID($h_array['sceneID']);
                $h_array['perdestrianOutsideType'] = $enc->decrypt_request($array['perdestrianOutsideType']);
                $h_array['hitAndRun'] = $enc->decrypt_request($array['hitAndRun']);
                $h_array['pedestrianType'] = $array['pedestrianType'];
                $h_array['numberOfCarsDroveOverBody'] = $enc->decrypt_request($array['numberOfCarsDroveOverBody']);
                $h_array['weatherType'] = $enc->decrypt_request($array['weatherType']);
                $h_array['weatherCondition'] = $enc->decrypt_request($array['weatherCondition']);
                $h_array['typeOfCar'] = $enc->decrypt_request($array['typeOfCar']);
                $h_array['anyWitnesses'] = $enc->decrypt_request($array['anyWitnesses']);
                $h_array['bodyMoved'] = $enc->decrypt_request($array['bodyMoved']);
                $h_array['victimJumped'] = $enc->decrypt_request($array['victimJumped']);
                $h_array['anyStrangeCircumstances'] = $enc->decrypt_request($array['anyStrangeCircumstances']);
